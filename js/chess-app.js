  ...
  if (line.startsWith('bestmove')) {
    ...
    if (match) {
      ...
      if (bestmove !== '(none)' && bestmove !== '0000') {
        ...
        $('#bestMoveDisplay').html('<div class="best-move-idle">Tidak ada gerakan</div>');
        $('#aiThinking').hide();
        isAnalyzing = false;
      }
    }
...
function getAIMove(autoPlay = false) {
  ...
  isAnalyzing = true;
  $('#aiThinking').show();
  $('#bestMoveDisplay').html('<div class="best-move-idle">Menganalisis posisi...</div>');
  $('#multiPVList').html('');
  ...
  let movesStr = '';
  if (history.length > 0) {
    movesStr = ' moves ' + history.map(m => m.from + m.to + (m.promotion || '')).join(' ');
  }
...
function renderRecommendation(bestUCI, linesData, turnIndicator) {
  ...
  const bestSAN   = uciToSAN(bestUCI);
  // Default to line 1 if available
  const topInfo   = linesData[1] || { score_cp: 0, mate_in: null };
  const scoreTxt  = formatScore(topInfo.score_cp, topInfo.mate_in);
  const color     = turnIndicator === 'w' ? '♙ Putih' : '♟ Hitam';

  $('#bestMoveDisplay').html(`
    <div class="best-move-info">
      <span class="bm-move">${bestSAN || bestUCI}</span>
      <div class="bm-meta">
        <span>${color} — Gerakan terbaik</span>
        <span class="bm-score">${scoreTxt}</span>
      </div>
    </div>
  `);

  // Multi-PV lines
  const sortedLines = Object.values(linesData);
  if (sortedLines.length > 1) {
    const html = sortedLines.map((line, i) => {
      if (!line.bestmove) return '';
      const san      = uciToSAN(line.bestmove);
      const sc       = formatScore(line.score_cp, line.mate_in);
      const scClass  = (line.score_cp || 0) > 20 ? 'positive' : (line.score_cp || 0) < -20 ? 'negative' : 'neutral';
      const pvPreview = (line.moves || []).slice(0, 4).join(' ');
      return `
        <div class="pv-item" onclick="applyAIMove('${line.bestmove}')">
...
function clearHighlights() {
  highlightSqs.forEach(sq => $(`[data-square="${sq}"]`).removeClass('highlight-from highlight-to'));
  highlightSqs = [];
}
...
function renderHistory() {
  ...
  moveHistory.forEach((pair, i) => {
    ...
    const isLastB = (i === moveHistory.length - 1 && game.turn() === 'w' || (!pair.black && i === moveHistory.length - 1));
...
function updateUI() {
  ...
  let status = '';
  if (game.in_checkmate())       status = `🏁 Skakmat! ${turn === 'w' ? 'Hitam' : 'Putih'} menang`;
...
$('#btnNewGame').on('click', () => {
  ...
  renderHistory();
  updateUI();
  $('#bestMoveDisplay').html('<div class="best-move-idle">Tunggu giliran...</div>');
  $('#multiPVList').html('');
...
$('#btnCopyPGN').on('click', () => {
  navigator.clipboard.writeText(game.pgn())
    .then(() => { const $b = $('#btnCopyPGN'); $b.text('✅ Copied!'); setTimeout(() => $b.text('📋 Copy PGN'), 2000); })
...
$('#eloRange').on('input', function () { $('#eloValue').text(this.value); });

$('#autoPlay').on('change', function () {
  if (this.checked && gameMode === 'suggest' && !game.game_over() && !isAnalyzing) {
    getAIMove(true);
  }
});
